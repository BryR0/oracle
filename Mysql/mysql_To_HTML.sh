...
echo "[client]" > $CNF
echo "user=$username" >> $CNF
echo "password=$password" >> $CNF
echo "host=$1" >> $CNF

CONN="mysql --defaults-file=$CNF"
...
echo "" > $TMPF/so.log
echo "<h1 class='title'>$1</h1><table class='responsive-table'>" >> $FILE_OUT
$2 > $TMPF/so.log
NLCC=0
sanear $TMPF/so.log
...
echo "" > $TMPF/dbt.log
echo "<h1 class='title'>ALL DATABASE</h1>" >> $FILE_OUT
...
echo "<h1 class='title'>BBDD: $DBS</h1>" >> $FILE_OUT
...
echo "<div id='stable'><p>$TDB</p><table class='responsive-table colaps' id='S_"$TDB"' style='display: none;'>" >> $FILE_OUT
